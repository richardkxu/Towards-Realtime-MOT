...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # id_text = "{}-{} [{:.2f}m,{:.2f}m]".format('person', obj_id, x1, y1)
        id_text = "[{}-{}]".format('person', obj_id)
        # t_size = cv2.getTextSize(id_text, cv2.FONT_HERSHEY_PLAIN, text_scale, text_thickness)[0]
        t_size = cv2.getTextSize(id_text, font, text_scale, text_thickness)[0]
        if ids2 is not None:
            id_text = id_text + ', {}'.format(int(ids2[i]))
        # _line_thickness = 1 if obj_id <= 0 else line_thickness
        # color = get_color(abs(obj_id))
        cv2.rectangle(im, intbox[0:2], intbox[2:4], color=mycolor['person'], thickness=line_thickness)
        cv2.putText(im, id_text.capitalize(), (intbox[0], intbox[1] + t_size[1] + 6), font, text_scale, mycolor['person'], text_thickness)
        # cv2.putText(im, id_text.capitalize(), (x1, y1 + t_size[1] + 4), cv2.FONT_HERSHEY_TRIPLEX, text_scale, (0, 0, 255), thickness=text_thickness)
    return im
